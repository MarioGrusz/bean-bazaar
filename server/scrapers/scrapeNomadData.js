...
const getNomadCoffeeProductData = async () => {
  ...
  await page.goto("https://nomadcoffee.es/en/collections/coffee");
  const productHandles = await page.$$(".collection-list > .grid > .item");
  ...
  for (const producthandle of productHandles) {
    try {
      const productImage = await producthandle.$eval(
        ".product-single__thumbnail-image",
        (el) => el.getAttribute("src")
      );
      const relativeLink = await producthandle.$eval("a", (el) =>
      ...
      const productPriceRange = await productPriceElement.evaluate((el) =>
        el.textContent.trim()
      );
      ...
      const productFirstNameElement = await producthandle.$(
        ".notranslate > span > h3"
      );
      const productFirstNameString = await productFirstNameElement.evaluate(
        (el) => el.textContent.trim()
      );
      const productSecondNameElement = await producthandle.$(
        ".notranslate > span > h3:nth-child(2)"
      );
      if (productSecondNameElement) {
        productSecondNameString = await productSecondNameElement.evaluate(
          (el) => el.textContent.trim()
        );
      } else {
        continue;
