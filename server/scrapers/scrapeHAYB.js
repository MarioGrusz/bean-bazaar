...
const getHAYBProductData = async () => {
  ...
  await page.goto("https://haybcoffee.eu/product-category/coffee/");
  const productData = [];
  ...
  for (const url of hrefs) {
    ...
    try {
      ...
      const productNameElement = await page.$(
        ".product-detail-content > .product-detail-value"
      );

      if (productNameElement) {
        productName = await productNameElement.evaluate((el) =>
          el.textContent.trim()
        );
      } else {
        continue;
